import { Share2, Shield, Copy, Eye, Plus, Globe } from "lucide-react"

export const features = [
  {
    icon: Shield,
    title: "Secure Authentication",
    description: "Safe sign up and sign in with JWT-based authentication to protect your content.",
  },
  {
    icon: Plus,
    title: "Easy Content Saving",
    description: "Simply paste YouTube or Twitter links with custom titles and organize by type.",
  },
  {
    icon: Share2,
    title: "Smart Sharing",
    description: "Share your entire collection with unique public links.",
  },
  {
    icon: Globe,
    title: "Public Access",
    description: "Generate shareable public pages that anyone can view without logging in.",
  },
  {
    icon: Copy,
    title: "One-Click Copy",
    description: "Instantly copy shareable links to your clipboard for easy distribution.",
  },
  {
    icon: Eye,
    title: "Dashboard View",
    description: "Clean, organized dashboard to view and manage all your saved content.",
  },
]

export const steps = [
  {
    step: "01",
    title: "Sign Up",
    description: "Create your account to get started with your personal content vault.",
  },
  {
    step: "02",
    title: "Add Content",
    description: "Paste YouTube or Twitter links, add titles, and organize by type.",
  },
  {
    step: "03",
    title: "Organize",
    description: "View all your saved content in a clean dashboard.",
  },
  {
    step: "04",
    title: "Share",
    description: "Generate unique public links to share your collection.",
  },
]
